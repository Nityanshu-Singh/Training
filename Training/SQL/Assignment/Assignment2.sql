...
Select Dept_Name, Dept_No From Department Where Dept_Name= 'Research';
 
-- Now we can use Department No. to Pay rise...
-- Before Pay Rise
Select Emp_no, Emp_Name, Job, salary From Employee where Dept_no = 20;
 
--After PayRise
select Emp_no, Emp_Name, Job, Salary = (Salary * 1.10) from Employee Where Dept_no = 20;
 
-- 5:- Find the number of CLERKS employed. Give it a descriptive heading.
Select COUNT(*) as "Number of the Employees as Clerk" from Employee where job = 'CLERK';
...
select Dept.Dept_No, Dept.Dept_Name, Dept.Loc FROM Department dept
...
select Dept.Dept_Name, Dept.Dept_no, sum(E.Salary) as "Total Salary" from Department Dept
...
select Emp_Name,Salary from Employee where Emp_Name='MILLER' or Emp_Name='SMITH'
 
--12:- Find out the names of the employees whose name begin with ‘A’ or ‘M’.

select Emp_Name from Employee where Emp_Name Like 'A%M'
 
--13:- Compute yearly salary of SMITH. 

select Emp_Name, Salary*12 as "Annual Salary" from Employee where Emp_Name ='Smith'
